...
def get_splits(y,):
    ...
    # 方案一：原先是按照对每类label取阈值个数据作为训练集，剩下的按1：2划分验证集以及测试集
    idx_train = []
    label_count = {}
    for i, label in enumerate(y):
        label = np.argmax(label)
        if label_count.get(label, 0) < 400:
            idx_train.append(i)
            label_count[label] = label_count.get(label, 0) + 1
    print('len of idx_list is ', len(set(idx_list)))
    idx_val_test = list(set(idx_list) - set(idx_train))
    print("len of idx_train is ", len(idx_train))
    val_len = int(len(idx_val_test)/3)
    idx_val = idx_val_test[:val_len]
    idx_test = idx_val_test[val_len:]
    print("len of idx_val is ", len(idx_val))
    print("len of idx_test is ", len(idx_test))

    # 方案二：按7：1：2的比例划分数据集
    # len_y = len(idx_list)
    # idx_train = idx_list[:int(0.7*len_y)]
    # idx_val = idx_list[int(0.7*len_y):int(0.8*len_y)]
    # idx_test = idx_list[int(0.8*len_y):]


    y_train = np.zeros(y.shape, dtype=np.int)
    y_val = np.zeros(y.shape, dtype=np.int)
    y_test = np.zeros(y.shape, dtype=np.int)
...
def load_data_v1(dataset="enron", path="../data/enron/",):

    idx_features_labels = np.genfromtxt("{}{}.content".format(path, dataset), dtype=np.dtype(str))
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float)
    onehot_labels = encode_onehot(idx_features_labels[:, -1])

    # build graph
    idx = np.array(idx_features_labels[:, 0], dtype=np.int)
    idx_map = {j: i for i, j in enumerate(idx)}
    edges_unordered = np.genfromtxt("{}{}.cites".format(path, dataset), dtype=np.int)
    edges = np.array(list(map(idx_map.get, edges_unordered.flatten())),
                     dtype=np.int).reshape(edges_unordered.shape)
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                        shape=(onehot_labels.shape[0], onehot_labels.shape[0]), dtype=np.float)
...
def encode_onehot(labels):
    ...
    labels_onehot = np.array(list(map(classes_dict.get, labels)), dtype=np.int)
